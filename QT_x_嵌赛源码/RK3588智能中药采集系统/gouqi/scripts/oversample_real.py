...
def oversample(copies: int = 40, prefix: str = "real_boost_") -> int:
    ...
    if not sources:
        print("未找到 real_*.jpg，请先 import_user_images.py")
        return 0
    clear_boost(prefix)
    n = 0
    for src in sources:
...
def main():
    ...
    p.add_argument("--copies", type=int, default=40, help="每张实拍复制份数")
    p.add_argument("--prefix", default="real_boost_")
    args = p.parse_args()
    n = oversample(args.copies, args.prefix)
    print(f"实拍增强: {n} 张 (约 {len(list(IMG_DIR.glob('real_[0-9]*.jpg')))} 原图 × {args.copies})")
